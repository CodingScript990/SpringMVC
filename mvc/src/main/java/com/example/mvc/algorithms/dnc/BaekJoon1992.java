package com.example.mvc.algorithms.dnc;
...
public class BaekJoon1992 {
    // 재귀함수 호출 => 입력에 대한 정보를 클래스 필드로 저장함
    private char[][] image; // 입력된 0과 1로 구성된 이미지
    public StringBuilder quadTreeBuilder; // 결과를 저장하기 위한 StringBuilder
    ...
    private void compressQuad(
        ...
        // 조건을 만족하였는지 검사하는 flag
        boolean success = true;
        // 모든 요소가 같지 않을 경우 success = false 를 해줌
        // x, y와 값을 저장해두고, x ~ x + n - 1, y ~ y + n - 1 까지 반복하면서
        // 초기의 값과 달라지는지를 검사함
        char init = image[x][y];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
