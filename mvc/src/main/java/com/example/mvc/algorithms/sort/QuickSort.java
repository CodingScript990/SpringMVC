...
public class QuickSort {
    ...
    private void quickSort(int[] arr, int low, int high) {
        if (low < high) {
            // quicksort 후 나눠진 index 를 반환받는다.
            int pivot = partition(arr, low, high);

            // 해당 index 를 기준으로 좌우에 대하여 다시
            // quicksort 를 호출한다.
    ...
    // Pivot 을 정하고, pivot 을 기준으로 좌우 배열의 원소들을 교환한 뒤 pivot 이 최종적으로 위치하는 곳을 반환하는 메서드
    private int partition(int[] arr, int low, int high) {
        ...
        // i + 1과 pivot 의 위치를 교환하면, i + 1 을 기준으로
        // 왼족은 pivot 보다 작은 값이
