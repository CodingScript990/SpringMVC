...
public class BFPatternMatching {
    ...
    public void solution() throws IOException {
        ...
        String target = reader.readLine(); // target string
        String pattern = reader.readLine(); // pattern string
        ...
        while (tarIdx < target.length() && patIdx < pattern.length()) {
            ...
            if (target.charAt(tarIdx) != pattern.charAt(patIdx)) {
                // TODO tarIdx 를 여태까지 이동한 만큼 되돌린다
                tarIdx -= patIdx;
                // TODO patIdx 를 -1로 할당한다
                patIdx = -1;
            }
            // TODO 다음칸으로 이동한다
            tarIdx += 1;
    ...
    // main method
    // swertyuiuiuytrertyuiopopoiuytrqwertyuytrertywqwertyuiuytrewqwertyuiiuiuiytqwert
    // qwert
