...
public class Kruskal {
    ...
    public void solution() throws IOException {
        ...
        // Kruskal 알고리즘은 서로소 집합의 개념을 이용해 서로다른 두 정점을 연결했을 때 사이클이 발생하는지 안하는지 구분함
        // 배열로 표현한 트리를 만듬
        ...
        for (int i = 0; i < edgeCount; i++) {
            ...
            // 2-1. start 와 end 를 연결한 간선을 택하면 사이클이 생기지 않는지? findSet, union
            if (findSet(start) != findSet(end)) {
                // 두 원소를 하나의 집합으로 간선을 골랐음을 표시함
                union(start, end);
                picked++;
                // 총 가중치 업데이트
            ...
            // 3. 고른 간선의 갯수가 N-1개가 될때까지
            if (picked == nodeCount - 1) break;
        }
        // output => Kruskal Algorithms result
    ...
    public int findSet(int node) {
        if (parent[node] != node) return findSet(parent[node]);
        else return parent[node];
    }

    // union : Y 가 속한 집합을 X 가 속한 집합에 합친다[합집합]
