...
public class BruteForce {
    ...
    public int solution() throws IOException {
        ...
        while (!toVisit.isEmpty()) {
            ...
            // 이번에 확인한 토마토가 익은 시점이 현재 기록된 시간보다 늦을 경우 갱신함
            ...
            for (int i = 0; i < 6; i++) {
                ...
                // 1. 영역을 벗어나지 않는다
                if (checkBounds(nextX, nextY, nextH)
                        // 2. 익지 않은 토마토이다
        ...
        // forh >> fory >> forx >> if tomato == 0 return -1;
    ...
    private boolean checkBounds(int x, int y, int h) {
        return -1 < x && x < this.x && -1 < y && y < this.y && -1 < h && h < this.h;
    }

    public static void main(String[] args) throws IOException {
       new BruteForce().solution();
    }
}
